...
from nornir_gitlab.plugins.tasks import gitlab
...
class Test(object):
    def test_gitlab_create_dry_run(self, nornir, requests_mock):
        nornir = nornir.filter(name="dev3.group_2")
        res = create_file(
            nornir=nornir,
        ...
        assert not res["dev3.group_2"][0].failed
        assert res["dev3.group_2"][0].changed
        assert res["dev3.group_2"][0].diff == diff_create
    ...
    def test_gitlab_create_file_exists(self, nornir, requests_mock):
        ...
        assert res["dev3.group_2"][0].failed
        assert not res["dev3.group_2"][0].changed
    ...
    def test_gitlab_update_dry_run(self, nornir, requests_mock):
        ...
        assert not res["dev3.group_2"][0].failed
        assert res["dev3.group_2"][0].changed
        assert res["dev3.group_2"][0].diff == diff_update
    ...
    def test_gitlab_get_dry_run(self, nornir, requests_mock):
        ...
        assert not res["dev3.group_2"][0].failed
        assert res["dev3.group_2"][0].changed
        assert res["dev3.group_2"][0].diff == diff
